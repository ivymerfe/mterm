...
std::wstring Utils::Utf8ToWChar(const std::string& utf8) {
  std::wstring wcharStr;
  size_t i = 0;
  while (i < utf8.size()) {
    wchar_t codepoint = 0;
    uint8_t byte = utf8[i];
    ...
    wcharStr.push_back(static_cast<wchar_t>(codepoint));
  }
  return wcharStr;
}

std::string Utils::WCharToUtf8(const std::wstring& wcharStr) {
  std::string utf8;
  for (wchar_t codepoint : wcharStr) {
    if (codepoint <= 0x7F) {
      // 1-byte sequence
      utf8.push_back(static_cast<char>(codepoint));
    } else if (codepoint <= 0x7FF) {
