...
class BaseApp(core.Window):
    ...
    def on_render(self):
        ...
        if self.active_terminal is not None:
            self.active_terminal.render(self.selector_width, y, self.terminal_width, height)
    ...
    def on_mousedown(self, button, x, y):
        if button == core.buttons.LEFT and self.selected_ctl_button == self.BUTTON_CAPTION:
            self.drag()
    ...
    def render_terminal_selector(self, x, y, width, height):
        ...
        for i, terminal in enumerate(self.terminals):
            ...
            self.render_selector_button(str(terminal.id), i+1, line_height, advance, x, y + i * button_height, width, is_hovered, is_active)
        ...
        self.render_selector_button("+", 0, line_height, advance, x, new_button_y, width, is_new_button_hovered, False)

    def render_selector_button(self, text, color_idx, line_height, advance, x, y, width, is_hovered, is_active):
        if is_active:
    ...
    def render_frame(self):
        ...
        if self.selected_ctl_button == self.BUTTON_CLOSE:
            self.rect(close_left, 0, close_left + full_size, caption_size, theme.Window.BUTTON_HOVER, 1)
        elif self.selected_ctl_button == self.BUTTON_MAXIMIZE:
            self.rect(max_left, 0, max_left + full_size, caption_size, theme.Window.BUTTON_HOVER, 1)
        elif self.selected_ctl_button == self.BUTTON_MINIMIZE:
            self.rect(min_left, 0, min_left + full_size, caption_size, theme.Window.BUTTON_HOVER, 1)
